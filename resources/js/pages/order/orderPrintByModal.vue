<template>
    <el-card class="order-container">
        ...
        <!-- Print Modal -->
        <el-dialog v-model="isPrintModalVisible" title="Print Preview" width="80%" center :style="{ marginTop: '5vh' }">
            <div class="modern-invoice">
                ...
                <section class="summary-section">
                    <div class="summary-grid">
                        ...
                        <div class="summary-label">Discount ({{ printableOrder?.discount || 0 }}%):</div>
                        <div class="summary-value discount">-{{ formatCurrency((printableOrder?.subTotal || 0) *
                            ((printableOrder?.discount || 0)/100)) }}</div>

                        <div class="summary-label">Tax:</div>
                        <div class="summary-value">{{ formatCurrency((printableOrder?.total || 0) - (printableOrder?.subTotal || 0) +
                            ((printableOrder?.subTotal || 0) * ((printableOrder?.discount || 0)/100))) }}</div>

                        <div class="summary-label grand-total">Total:</div>
                        <div class="summary-value grand-total">{{ formatCurrency(printableOrder?.total || 0) }}</div>

                        <div class="summary-label">Amount Paid:</div>
                        <div class="summary-value">{{ formatCurrency(printableOrder?.paid || 0) }}</div>

                        <div class="summary-label">Balance Due:</div>
                        <div class="summary-value">{{ formatCurrency(printableOrder?.due || 0) }}</div>
                    </div>

                    <div class="payment-method">
                        <p><strong>Payment Method:</strong> {{ printableOrder?.payby || 'N/A' }}</p>
                    </div>
                </section>
            </div>
            <template #footer>
                <div class="dialog-footer">
                    <el-button @click="printInvoice" type="primary">
                        <el-icon>
                            <Printer />
                        </el-icon> Print
                    </el-button>
                    <el-button @click="isPrintModalVisible = false" type="danger">Close</el-button>
                </div>
            </template>
        </el-dialog>
    </el-card>
